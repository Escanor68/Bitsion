...
describe('PersonaService', () => {
  ...
  describe('create', () => {
    it('should create persona with valid data', async () => {
      ...
      const createdPersona = createMockPersona(
        3,
        createDto.nombreCompleto,
        createDto.identificacion,
        createDto.edad,
        createDto.genero,
        createDto.estado || 'Activo'
      );
      ...
      expect(mockRepository.existsByIdentificacion).toHaveBeenCalledWith(
        createDto.identificacion
      );
    ...
    it('should throw error when identificacion already exists', async () => {
      ...
      await expect(personaService.create(createDto)).rejects.toThrow(
  ...
  describe('update', () => {
    it('should update persona with valid data', async () => {
      const existingPersona = createMockPersona(
        1,
        'Juan Pérez',
        '12345678',
        30,
      ...
      mockRepository.update.mockResolvedValue({
        ...existingPersona,
        ...updateDto,
        atributosAdicionales: JSON.stringify(updateDto.atributosAdicionales),
      });
    ...
    it('should throw error when persona not found', async () => {
      ...
      await expect(personaService.update(999, updateDto)).rejects.toThrow(
    ...
    it('should throw error when identificacion already exists for another persona', async () => {
      ...
      await expect(personaService.update(1, updateDto)).rejects.toThrow(
        'Ya existe otra persona con esta identificación'
      );
    });
  });
  ...
  describe('delete', () => {
    ...
    it('should throw error when persona not found', async () => {
      ...
      await expect(personaService.delete(999)).rejects.toThrow(
        'No se encontró la persona con ID: 999'
      );
    });
  });
